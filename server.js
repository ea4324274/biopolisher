...
app.post("/api/polish-bio", async (req, res) => {
  try {
    ...
    if (!bio || typeof bio !== "string" || bio.trim().length < 10) {
      return res.status(400).json({ error: "Please provide a valid bio (>= 10 characters)." });
    }

    const lengthRule = short ? "Limit to 150 words." : "";
    const system = "You are a copywriter who helps mental health professionals write approachable bios.";
    const user = `
...
Respond with just the polished bio, no preface.
    ...
    if (!output) return res.status(502).json({ error: "No output from model." });
